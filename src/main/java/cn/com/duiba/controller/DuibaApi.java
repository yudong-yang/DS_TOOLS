...
import cn.com.duiba.ds.tools.sdk.RecordParams;
...
//			System.out.println("加积分响应内容==：" + ccr.toString());
		...
		try {
			...
			if (params.isSuccess()) {
				...
				if (orderstatus == "success") {
					...
					String msg = userService.returnCredit(Credit.getUserId(),
							Credit.getCredits());
					...
					logger.info("用户积分返还情况====：积分已经返还，请勿重复返还");
				}
				return "ok";
			}
		} catch (Exception e) {
	...
	@RequestMapping("/recordInfo/{appid}")
	@ResponseBody
	@Transactional
	public String recordInfo(HttpServletRequest request,
		...
		CreditTool tool = new CreditTool(appKey, appSecret);
		try {
			RecordParams params = tool.RecordParams(request);
			logger.info("兑换记录信息=="+params.getTitle()+"=="+params.getRecordId()+"=="+params.getRecordDetailUrl());
	...
	public String virtual(HttpServletRequest request,
		...
		try {
			...
			System.out.println("虚拟商品标示符======" + virparam + "====账号==="
					+ account);
			...
			if (virparam.equals("33")) {
				...
				result.setCredits(userService.GetCreditsByUid(uid));
				return result.toString();
			}else if (uid.endsWith("4444")) {
				VirtualConsumeResult result = new VirtualConsumeResult("process");
				result.setErrorMessage("");
				result.setSupplierBizId(supplierBizId);
				result.setCredits(userService.GetCreditsByUid(uid));
				return result.toString();
			} else {
				VirtualConsumeResult result = new VirtualConsumeResult(
						"success");
				result.setSupplierBizId(supplierBizId);
				result.setCredits(userService.GetCreditsByUid(uid));
	...
	public RedirectView autologin(HttpServletRequest request,
			@CookieValue("cook") String cookvalue,
			@PathVariable("appid") String appid) {
		{
